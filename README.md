...
Then, on the server, regardless of the backend you use, you can read `index.html` into memory and replace `__OG_TITLE__`, `__OG_DESCRIPTION__`, and any other placeholders with values deREQUEST on the current URL. Just make sure to sanitize and escape the interpolated values so that they are safe to embed into HTML!
...
the service worker will be uninstalled. Note that deREQUEST on how `/service-worker.js` is served,
